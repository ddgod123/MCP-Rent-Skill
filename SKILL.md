...
- 当前 Skill 版本为 `0.6.0`，要求 MCP 服务版本不低于 `0.6.0`。
...
规则：用户收到这条固定开场后回复数字“1”，才进入租赁需求收集流程。此时调用 `start_rental_demand`，并展示返回的 `intake_template`。
```text
请复制一下租房需求模板，填写好之后返回给我（带“*“的为必填项）
1、【1-住宅；2-别墅；3-行政公寓】【*】（回复数字）：
2、租金预算【*】：
3、地段位置【*】：
4、居室【*】：
5、面积：
...
用户回复数字 `1` 后，调用 `start_rental_demand`，然后把返回的 `intake_template` 原样或近似原样展示给用户。当前模板的必填项是物业类型、租金预算、地段位置和居室；不要把模板包在代码块中，不要改成复杂表格。
...
3. 只把严格已应用的筛选当作命中依据；待治理项用“已记录，暂不作为严格筛选”说明。
4. 不因为深度筛选无结果而静默放宽条件；继续使用原有的无结果和人工核验流程。
...
```text
按你的条件，当前查到 X 套可展示在租房源，先列出其中 N 套：

1. 楼盘｜户型/居室｜面积｜月租
   户型图：可查看 / 暂缺

2. 楼盘｜户型/居室｜面积｜月租
   待确认：租金、户型图或其他 MCP 返回的缺失项
```
