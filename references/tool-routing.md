...
- **沿线口径**：按楼盘登记点距环路 400 米的业务规则判定，350 至 450 米临界带暂待核实，不等同于楼栋或大门到道路的距离。只采用当前 `matched_facts` 或详情 `transport.ring_road`；定位、证据或几何版本变化可能让旧结果退出，不能拿历史回答补回。对客不展示几何版本、坐标或内部证据。
