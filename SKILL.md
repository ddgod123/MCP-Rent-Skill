...
- 当前 Skill 版本以文件头部 `metadata.version` 为准，要求 MCP 服务版本不低于 `metadata.mcp_min_version`。
- `SKILL.md` 是静态行为文件，不能自行联网、检查版本、修改本地文件或重载 WorkBuddy。
- 更新分两层：
  1. MCP 只读检查：宿主可在启动或 RentPro Skill 首次加载时调用一次 `check_rentpro_skill_update`，传入
     已加载文件的 `metadata.version` 作为 `current_skill_version`；该工具只比较当前 MCP 声明的兼容版本并
     返回 manifest 地址与更新命令，不联网核查 GitHub 最新发布、不下载、不修改文件。不要在每轮租客咨询中重复检查。
  2. 本地脚本执行：宿主具备执行本地命令能力时，使用项目中的 `scripts/rentpro_skill_update.py` 完成实际
     拉取、SHA-256 校验与替换；不应直接跟随未校验的 `main` 文件。
- 菜单 `9` 是面向用户的「检查更新 Skill」入口（见第 2 节），仅在用户主动发送数字 `9` 时触发：
  1. 宿主不具备本地命令执行能力时，回复「无法自动更新，请联系管理员手动执行更新。」，不声称已经更新。
  2. 宿主具备本地命令执行能力时，先执行 `scripts/rentpro_skill_update.py check` 比较远端版本：
     - 当前已是最新：回复「当前已是最新版本，无需更新。」
     - 有可用新版本：执行 `scripts/rentpro_skill_update.py update --yes` 拉取最新 Skill 包，下载前必须
       校验固定仓库来源、Skill 名称、版本号和 SHA-256；成功后回复「已更新到最新版本，请重启 WorkBuddy
       或刷新 Skill 后生效。」
  3. 更新失败、校验失败或网络不可用时，回复「更新失败，请稍后重试或联系管理员。」，不重复自动重试。
- 更新命令、版本号、GitHub 地址等实现细节不进入普通租客的业务回复，只在用户主动选择 `9` 时执行并给出简短结果。
...
- 菜单 `1` 只负责开启或继续租赁咨询；`2` 是查询当前业务楼盘，`3` 是查看帮助手册，
  `6` 是联系房源经理（直接给出房源经理联系方式），`9` 是检查更新 Skill（仅在用户主动选择时执行）。
  不把 `1` 当成所有业务的统一入口，也不要求用户先填写租房需求才能选择 `2`、`3`、`6` 或 `9`。
...
- 命中 `1`–`9` 但该编号尚未定义（当前 `4`、`5`、`7`、`8` 为空）时，不调用任何 MCP 工具，
...
“现在有哪些在租/出租房源”“有什么房子”“目前还有哪些可租”等当前挂牌问题时：

- 直接调用 `search_listings`，传 `property_name=单个楼盘名称`、`mode=list`；不调用
  `start_rental_demand`，不展示模板，不追问预算、物业类型、区域或居室等必填项。
- `property_name` 只用于楼盘名称、别名或英文名的精确匹配（忽略空白、大小写并支持中文数字写法），
  不把房源标题、地址或户型命中误当成楼盘命中。以 MCP 返回的 `property_groups[].property_name` 和
  每套房源的真实楼盘字段确认展示对象。
- 一次只展示当前楼盘的有效期、楼盘资料和该楼盘全部返回挂牌；不要展示其他楼盘聚合表，
  不把直查结果写成已建立、已确认或已修改的租房需求。
- 已有 `demand_id`、需求版本、筛选条件或收藏/排除上下文时，直查是一次性旁路查询：保留原需求，
  不覆盖、不清空、不更新需求版本；用户随后说“按我的需求找房”时恢复 `search_listings_by_demand`。
- 用户一条消息明确提到多个楼盘时，宿主按原始提及顺序去重建立当前对话的 `direct_property_queue`，
  首轮只查询第一个楼盘。回复末尾提示“回复‘下一个’查看下一个楼盘，也可以直接发送具体楼盘名称”。
  不把多个楼盘拼入一次 `keyword` 或 `property_name`，不预先向客户展示后续楼盘。
- 用户回复“下一个”时查询队列中尚未展示的下一个楼盘；用户直接发送队列中的楼盘名时跳转到该楼盘并
  从队列移除；不在队列中的具体楼盘名直接开启新的直查。队列成功返回（包括零结果）后才移动游标，
  查询失败或名称歧义时保留当前项。队列仅属于当前对话，不复用到新对话，返回菜单不清除。
- 队列为空时说明当前楼盘已展示完，提示用户可直接发送其他楼盘名称；不要循环或自动改查其他区域。
- 直查不要求先填写模板，也不因用户没有收到模板而拒绝回答。楼盘不在 MCP 当前发布范围或没有可展示挂牌时，
  按现有无结果口径说明“当前发布范围没有可直接展示的在租房源”，不编造全市场结论。
...
- 直查队列：当前对话的 `direct_property_queue`、`direct_property_cursor` 和当前直查楼盘；仅由宿主维护，
  不写入需求版本。直查使用的 `session_token` 只用于当前匿名浏览工作区，不替代需求会话令牌。
...
标准租房流程只有在当前需求清单 `search_ready=true` 时才搜索。具体楼盘直查是独立旁路，不建立需求且不受
该门禁限制：
...
- 如果这句话同时点名具体楼盘，按第 2.1A 直接调用 `search_listings(property_name=..., mode=list)`，
  不先走 `start_rental_demand`。
...
- 具体楼盘在租房源直查优先于本节：调用 `search_listings(property_name=..., mode=list)`，不创建或修改需求。
